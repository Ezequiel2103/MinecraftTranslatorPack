...
def main():
    parser = argparse.ArgumentParser(
        description="Optional live DeepSeek translation smoke test."
    )
    parser.add_argument(
        "--live",
        action="store_true",
    ...
    if not os.getenv("DEEPSEEK_API_KEY"):
        raise SystemExit(
            "DEEPSEEK_API_KEY is not set. Configure it before using --live."
        )

    service = TranslationService(
        "en_es",
        ai_translator=DeepSeekTranslator(model=args.model)
    )
