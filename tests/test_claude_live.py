...
def main():
    parser = argparse.ArgumentParser(
        description="Optional live Claude translation smoke test."
    )
    parser.add_argument(
        "--live",
        action="store_true",
    ...
    if not os.getenv("ANTHROPIC_API_KEY"):
        raise SystemExit(
            "ANTHROPIC_API_KEY is not set. Configure it before using --live."
        )

    service = TranslationService(
        "en_es",
        ai_translator=ClaudeTranslator(model=args.model)
    )
